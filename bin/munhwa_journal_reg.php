...
function proc_feed($zip_fname)
{
	...
	if ($res === true) {
		...
		//removeDir('C:/www.korea-copy.com/minju/munhwa_zip_tmp');
		mkdir('C:/www.korea-copy.com/minju/munhwa_zip_tmp');
		$zip->extractTo('C:/www.korea-copy.com/minju/munhwa_zip_tmp/');
		$zip->close();

		$xmlfname = "Munhwa 2020-1.xml"; ////////////////////////////////////////////////////////////////////
		...
		if (file_exists("C:/www.korea-copy.com/minju/munhwa_zip_tmp/".$xmlfname)) {
			print "[FOUNT FILE ($xmlfname)]\n";
			
			$dom = new DOMDocument('1.0', 'UTF-8');
			$dom->load("C:/www.korea-copy.com/minju/munhwa_zip_tmp/".$xmlfname);

			$root = $dom->getElementsByTagName('topics')->item(0);
			$topic = $root->getElementsByTagName('topic');
			$sqllist = array();
			foreach ($topic as $node) {
				print "[MUNHWA JOURNAL ARTICLE FOUNT]\n";
				...
				$sqllist[] = array(
					'sql' => $sql,
					'params' => array('dummy', $title, $WriterName, $Nayong1, $Nayong2, '2018', $BalHengYear, $GwonHo, '0', '', '', '1')
				);
			}

			$journal_pdf_fname = sprintf("C:/www.korea-copy.com/minju/MunHwa %04d.%d.pdf", $BalHengYear, $GwonHo);
			if (file_exists($journal_pdf_fname)) {
				...
				$FileName = sprintf("/Uploaded/Journal/2018/KPJ_%d.pdf", $max_articleid);
				$FileSize = filesize($journal_pdf_fname);

				$title = "문화어학습 PDF ".$BalHengYear."-".$GwonHo;

				$params = array($max_articleid, $title, '', '', '', '2018', $BalHengYear, $GwonHo, '0', $FileName, $FileSize, '1');

				$ret = sqlsrv_query($msdb, $sql, $params);
				if ($ret === false) {
					//print var_export(sqlsrv_errors(), true) . "\n";
					file_put_contents('munhwa_journal_err.txt', var_export(sqlsrv_errors(), true), FILE_APPEND);
					file_put_contents('munhwa_journal.txt', var_export($sql['params'], true), FILE_APPEND);
				}
				file_put_contents('munhwa_journal.txt', var_export($sql, true), FILE_APPEND);
				file_put_contents('munhwa_journal.txt', var_export($params, true), FILE_APPEND);
			...
			foreach ($sqllist as $sql) {
				...
				file_put_contents('munhwa_journal.txt', var_export($sql['sql'], true), FILE_APPEND);
				file_put_contents('munhwa_journal.txt', var_export($sql['params'], true), FILE_APPEND);
			...
			//_sendmail2('suyama@btl.co.jp', 'suyama@btl.co.jp', "C:/www.korea-copy.com/minju/munhwa_zip_tmp/".$xmlfname);
...
{
	$mailTo      = $to;
	$mailSubject = 'MunHwa Journal Article '.$file;
	$mailMessage = 'MunHwa Journal Article '.$file;
