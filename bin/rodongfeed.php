...
function main()
{
	...
	if (file_exists("C:/www.korea-copy.com/ftpdata/rodong_pdf_complete.txt") && file_exists("C:/www.korea-copy.com/ftpdata/".$pdf_fname)) {
		//---------------------------------------------------------------------
		// PDF Feed
		//---------------------------------------------------------------------
		$yyyy = date("Y");
		...
		// PDF upload
		//---------------------------------------------------------------------
		touch("C:/www.korea-copy.com/ftpdata/pdf_uploaded_".date('Ymd'));
		
		proc_ftp_for_newrodong($pdf_fname, null, null);
		$ftp_ret = proc_ftp_for_korea1($pdf_fname, $zip_fname, $pic_fname);
		//proc_ftp($pdf_fname, $zip_fname, $pic_fname);
		//proc_mail($pdf_fname, $zip_fname, $pic_fname);

		$logstr = sprintf("%s KOREA1 FTP (status=%s).\n", date("Y/m/d H:i:s"), $ftp_ret?"true":"false");
		file_put_contents('test_0.sql', $logstr, FILE_APPEND);

		$logstr = sprintf("%s PDF sent.\n", date("Y/m/d H:i:s"));
		file_put_contents('test_0.sql', $logstr, FILE_APPEND);

		rename("C:/www.korea-copy.com/ftpdata/".$pdf_fname, "C:/www.korea-copy.com/finish/".$pdf_fname);
		unlink("C:/www.korea-copy.com/ftpdata/pdf_uploaded_".date('Ymd'));
		unlink("C:/www.korea-copy.com/ftpdata/rodong_pdf_complete.txt");
	}

	if (file_exists("C:/www.korea-copy.com/ftpdata/rodong_zip_complete.txt") && file_exists("C:/www.korea-copy.com/ftpdata/".$zip_fname)) {
		//---------------------------------------------------------------------
		// XML Feed
		//---------------------------------------------------------------------
		$ret = proc_feed($zip_fname);
		if ($ret == false) {
			touch("C:/www.korea-copy.com/ftpdata/zip_err");
			
			$logstr = sprintf("%s XML not feeded.\n", date("Y/m/d H:i:s"));
			file_put_contents('test_0.sql', $logstr, FILE_APPEND);
		} else {
			$logstr = sprintf("%s XML feeded.\n", date("Y/m/d H:i:s"));
			file_put_contents('test_0.sql', $logstr, FILE_APPEND);
			
			if (file_exists("C:/www.korea-copy.com/ftpdata/zip_err")) {
				unlink("C:/www.korea-copy.com/ftpdata/zip_err");
			}
		}
		...
		// ZIP upload
		//---------------------------------------------------------------------
		if (!file_exists("C:/www.korea-copy.com/ftpdata/zip_uploaded_".date('Ymd'))) {
			touch("C:/www.korea-copy.com/ftpdata/zip_uploaded_".date('Ymd'));
			proc_ftp_for_newrodong(null, $zip_fname, null);
			
			$logstr = sprintf("%s PDF sent.\n", date("Y/m/d H:i:s"));
			file_put_contents('test_0.sql', $logstr, FILE_APPEND);
		} else {
			$logstr = sprintf("%s PDF not send (zip err).\n", date("Y/m/d H:i:s"));
			file_put_contents('test_0.sql', $logstr, FILE_APPEND);
		}

		if (!file_exists("C:/www.korea-copy.com/ftpdata/zip_err")) {
			rename("C:/www.korea-copy.com/ftpdata/".$zip_fname, "C:/www.korea-copy.com/finish/".$zip_fname);
			unlink("C:/www.korea-copy.com/ftpdata/zip_uploaded_".date('Ymd'));
			unlink("C:/www.korea-copy.com/ftpdata/rodong_zip_complete.txt");
		}
	}

	if (file_exists("C:/www.korea-copy.com/ftpdata/rodong_pic_complete.txt") && file_exists("C:/www.korea-copy.com/ftpdata/".$pic_fname)) {
		//---------------------------------------------------------------------
		// PIC upload
		//---------------------------------------------------------------------
		touch("C:/www.korea-copy.com/ftpdata/pic_uploaded_".date('Ymd'));
		proc_ftp_for_newrodong(null, null, $pic_fname);

		$logstr = sprintf("%s PIC sent.\n", date("Y/m/d H:i:s"));
		file_put_contents('test_0.sql', $logstr, FILE_APPEND);
		
		rename("C:/www.korea-copy.com/ftpdata/".$pic_fname, "C:/www.korea-copy.com/finish/".$pic_fname);
		unlink("C:/www.korea-copy.com/ftpdata/pic_uploaded_".date('Ymd'));
		unlink("C:/www.korea-copy.com/ftpdata/rodong_pic_complete.txt");
	}

	$trans_csv_fname = date("Ymd") . "J.csv";
