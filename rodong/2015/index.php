...
foreach($pdflist as $idx => $val){
	...
	if (!in_array($matches[3], $monthlist)){
		$monthlist[] = (int)$matches[3];
	}
if ((int)$matches[3] == 12 && (int)$matches[4] == 18){
//var_dump($matches[5]);
}
	//if (!isset($daypdfnum[(int)$matches[3]][(int)$matches[4]])){
		if ((int)$daypdfnum[(int)$matches[3]][(int)$matches[4]] < (int)$matches[5]) {
			$daypdfnum[(int)$matches[3]][(int)$matches[4]] = (int)$matches[5];
		}
	//}
}
